...
test('SSH 标题栏显示 Alias、原始资产名、ID 和状态灯，并按 OSC 7 启用路径复制', async () => {
  ...
  expect(sftp.nextElementSibling).toBe(upload)
  expect(upload.nextElementSibling).toBe(download)
  expect(upload).toBeDisabled()
  expect(download).toBeDisabled()
  await user.click(sftp)
  expect(backend.startSFTPSession).not.toHaveBeenCalled()
...
test('SSH 发起 SFTP 固定原会话身份并只读取一次工作目录', async () => {
  ...
  expect(connect).toHaveAttribute('title', '连接SFTP')
  expect(connect.previousElementSibling).toHaveClass('terminal-action-separator')
  expect(connect.nextElementSibling).toBe(screen.getByRole('button', { name: '上传文件（ZMODEM）' }))
...
test('可见的未选中资产也加载协议，快速连接保留仅支持 SFTP 的资产', async () => {
  ...
  const dialog = within(screen.getByRole('dialog', { name: '快速连接' }))
  expect(dialog.getByRole('button', { name: 'files：连接 SSH' })).toBeDisabled()
  await userEvent.type(dialog.getByPlaceholderText('名称、地址、Asset ID 或 Alias'), 'files')
  await userEvent.keyboard('{Enter}')
  expect(backend.startSSHSession).not.toHaveBeenCalled()
  await userEvent.click(dialog.getByRole('button', { name: 'files：连接 SFTP' }))
  await waitFor(() => expect(backend.startSFTPSession).toHaveBeenCalledTimes(1))
  expect(backend.startSSHSession).not.toHaveBeenCalled()
})
...
test.each(['{Enter}', '{Shift>}{Enter}{/Shift}'])('快速连接唯一结果在搜索框用 %s 连接对应协议', async (keys) => {
  const backend = makeBackend({ getAsset: vi.fn().mockResolvedValue({ ...assetDetail, protocols: [{ name: 'ssh', port: 22 }, { name: 'sftp', port: 22 }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  const dialog = within(screen.getByRole('dialog', { name: '快速连接' }))
  const input = dialog.getByPlaceholderText('名称、地址、Asset ID 或 Alias')
  expect(input).toHaveFocus()
  await user.type(input, 'production-web')
  expect(dialog.getAllByRole('row')).toHaveLength(1)
  expect(dialog.getByText('（deploy）')).toBeInTheDocument()
  expect(dialog.queryByText(/account-1/)).not.toBeInTheDocument()
  await user.keyboard(keys)
  const sftp = keys.includes('Shift')
  await waitFor(() => expect(sftp ? backend.startSFTPSession : backend.startSSHSession).toHaveBeenCalledWith(expect.objectContaining({ target: 'production-web', account: 'account-1' })))
  expect(sftp ? backend.startSSHSession : backend.startSFTPSession).not.toHaveBeenCalled()
  await waitFor(() => expect(screen.queryByRole('dialog', { name: '快速连接' })).not.toBeInTheDocument())
})

test('快速连接上下选择后继续用键盘选择账号，账号弹窗不被遮挡', async () => {
  const backend = makeBackend()
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  ...
  expect(input).toHaveFocus()
  expect(dialog.getAllByRole('row')[1]).toHaveAttribute('aria-selected', 'true')
  await user.keyboard('{Enter}')
  const accounts = await screen.findByRole('dialog', { name: '选择连接账号' })
  expect(screen.queryByRole('dialog', { name: '快速连接' })).not.toBeInTheDocument()
  expect(within(accounts).getByRole('button', { name: /deploy/ })).toHaveFocus()
  await user.keyboard('{ArrowDown}{Enter}')
  await waitFor(() => expect(backend.startSSHSession).toHaveBeenCalledWith(expect.objectContaining({ target: 'web-any', account: 'account-2' })))
})

test('快速连接过滤重置选中项，无结果和输入法确认不会连接，Esc 关闭', async () => {
  const backend = makeBackend()
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  ...
  expect(dialog.getAllByRole('row')[0]).toHaveAttribute('aria-selected', 'true')
...
test.each(['主体', 'SSH', 'SFTP'])('快速连接点击 %s 只发起对应连接', async (action) => {
  const backend = makeBackend({ getAsset: vi.fn().mockResolvedValue({ ...assetDetail, protocols: [{ name: 'ssh', port: 22 }, { name: 'sftp', port: 22 }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  const dialog = within(screen.getByRole('dialog', { name: '快速连接' }))
  await user.click(action === '主体' ? await dialog.findByText('production-web') : await dialog.findByRole('button', { name: `production-web：连接 ${action}` }))
  await waitFor(() => expect(action === 'SFTP' ? backend.startSFTPSession : backend.startSSHSession).toHaveBeenCalledTimes(1))
  expect(action === 'SFTP' ? backend.startSSHSession : backend.startSFTPSession).not.toHaveBeenCalled()
})

test('快速连接绑定账号失效时不显示 ID，也不改用其他账号', async () => {
  const backend = makeBackend({ getAsset: vi.fn().mockResolvedValue({ ...assetDetail, accounts: [assetDetail.accounts[1]] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  const dialog = within(screen.getByRole('dialog', { name: '快速连接' }))
  await user.type(dialog.getByPlaceholderText('名称、地址、Asset ID 或 Alias'), 'production-web')
  expect(dialog.getByText('绑定账号不可用')).toBeInTheDocument()
  expect(dialog.queryByText(/account-1/)).not.toBeInTheDocument()
  expect(dialog.getByRole('button', { name: 'production-web：连接 SSH' })).toBeDisabled()
  await user.keyboard('{Enter}')
  expect(backend.startSSHSession).not.toHaveBeenCalled()
})

test('快速连接在准备连接期间忽略重复回车和按钮点击', async () => {
  let finish!: (state: SessionState) => void
  const backend = makeBackend({ startSSHSession: vi.fn(() => new Promise<SessionState>((resolve) => { finish = resolve })) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  ...
  expect(backend.startSSHSession).toHaveBeenCalledTimes(1)
  expect(dialog.getByRole('button', { name: 'production-web：连接 SSH' })).toBeDisabled()
  await user.click(dialog.getByText('production-web'))
  expect(backend.startSSHSession).toHaveBeenCalledTimes(1)
  await act(async () => finish({ id: 'pending-quick', status: 'active', title: 'production-web', profile: 'production', organization: 'org-1', asset: 'asset-1', account: 'account-1', error: '' }))
  expect(screen.queryByRole('dialog', { name: '快速连接' })).not.toBeInTheDocument()
})

test('快速连接用账号用户名解析别名绑定，仅显示括号内的登录账号', async () => {
  const asset = { ...assetPage.results[0], aliases: [{ ...assetPage.results[0].aliases[0], account: 'deploy' }] }
  const backend = makeBackend({ listAssets: vi.fn().mockResolvedValue({ ...assetPage, results: [asset] }), getAsset: vi.fn().mockResolvedValue({ ...assetDetail, ...asset, accounts: [{ ...assetDetail.accounts[0], name: '部署账号' }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  ...
  await user.type(dialog.getByPlaceholderText('名称、地址、Asset ID 或 Alias'), 'production-web')
  expect(dialog.getByText('（deploy）')).toBeInTheDocument()
  expect(dialog.queryByText(/部署账号/)).not.toBeInTheDocument()
  await user.keyboard('{Enter}')
  await waitFor(() => expect(backend.startSSHSession).toHaveBeenCalledWith(expect.objectContaining({ target: 'production-web', account: 'deploy' })))
})

test('快速连接从唯一资产用键盘选择 SFTP 账号，取消后返回原搜索', async () => {
  const backend = makeBackend({ getAsset: vi.fn().mockResolvedValue({ ...assetDetail, protocols: [{ name: 'ssh', port: 22 }, { name: 'sftp', port: 22 }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  await user.type(screen.getByPlaceholderText('名称、地址、Asset ID 或 Alias'), 'web-any')
  await user.keyboard('{Shift>}{Enter}{/Shift}')
  await screen.findByRole('dialog', { name: '选择连接账号' })
  await user.keyboard('{Escape}')
  const input = await screen.findByPlaceholderText('名称、地址、Asset ID 或 Alias')
  expect(input).toHaveValue('web-any')
  expect(input).toHaveFocus()
  await user.keyboard('{Shift>}{Enter}{/Shift}')
  await screen.findByRole('dialog', { name: '选择连接账号' })
  await user.keyboard('{ArrowDown}{Enter}')
  await waitFor(() => expect(backend.startSFTPSession).toHaveBeenCalledWith(expect.objectContaining({ target: 'web-any', account: 'account-2' })))
  expect(backend.startSSHSession).not.toHaveBeenCalled()
})

test('快速连接关闭按钮获得焦点后仍可用 Esc 关闭', async () => {
  const user = userEvent.setup()
  render(<App backend={makeBackend()} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  within(screen.getByRole('dialog', { name: '快速连接' })).getByRole('button', { name: '关闭' }).focus()
  await user.keyboard('{Escape}')
  expect(screen.queryByRole('dialog', { name: '快速连接' })).not.toBeInTheDocument()
})

test.each(['空白处', 'SFTP 按钮', '关闭按钮'])('快速连接焦点位于%s时上下键仍选择结果，Enter 使用选中目标连接 SSH', async (focus) => {
  const asset = { ...assetPage.results[0], aliases: assetPage.results[0].aliases.map((alias, index) => ({ ...alias, account: `account-${index + 1}` })) }
  const backend = makeBackend({ listAssets: vi.fn().mockResolvedValue({ ...assetPage, results: [asset] }), getAsset: vi.fn().mockResolvedValue({ ...assetDetail, ...asset, protocols: [{ name: 'ssh', port: 22 }, { name: 'sftp', port: 22 }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  const dialog = within(screen.getByRole('dialog', { name: '快速连接' }))
  if (focus === '空白处') await user.click(dialog.getByRole('heading'))
  else act(() => dialog.getByRole('button', { name: focus === 'SFTP 按钮' ? 'production-web：连接 SFTP' : '关闭' }).focus())
  expect(dialog.getByRole('combobox')).not.toHaveFocus()
  await user.keyboard('{ArrowDown}{ArrowDown}{ArrowUp}')
  expect(dialog.getAllByRole('row')[1]).toHaveAttribute('aria-selected', 'true')
  await user.keyboard('{Enter}')
  await waitFor(() => expect(backend.startSSHSession).toHaveBeenCalledWith(expect.objectContaining({ target: 'web-any', account: 'account-2' })))
...
test('快速连接失焦后 Shift+Enter 仍连接 SFTP，关闭后释放键盘事件', async () => {
  const backend = makeBackend({ getAsset: vi.fn().mockResolvedValue({ ...assetDetail, protocols: [{ name: 'ssh', port: 22 }, { name: 'sftp', port: 22 }] }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  await user.click(within(screen.getByRole('dialog', { name: '快速连接' })).getByRole('heading'))
  await user.keyboard('{Shift>}{Enter}{/Shift}')
  await waitFor(() => expect(backend.startSFTPSession).toHaveBeenCalledTimes(1))
  expect(backend.startSSHSession).not.toHaveBeenCalled()
  await waitFor(() => expect(screen.queryByRole('dialog', { name: '快速连接' })).not.toBeInTheDocument())
...
test('快速连接失焦后仍拦截上下键默认滚动，忽略输入法确认并支持 Esc', async () => {
  const backend = makeBackend()
  const user = userEvent.setup()
  render(<App backend={backend} />)
  await screen.findByTestId('asset-row-asset-1')
...
test('快速连接被其他弹窗覆盖时不截获其键盘操作，返回后恢复快捷键', async () => {
  let requestQuit = () => undefined
  const backend = makeBackend({ onQuitRequested: vi.fn((handler) => { requestQuit = handler; return () => undefined }) })
  const user = userEvent.setup()
  render(<App backend={backend} />)
  ...
  await user.click(screen.getByRole('button', { name: '新建连接' }))
  act(() => requestQuit())
  const quit = within(screen.getByRole('dialog', { name: '停止传输并退出？' }))
  act(() => quit.getByRole('button', { name: '取消' }).focus())
  const event = new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true, cancelable: true })
  fireEvent(document.activeElement!, event)
  expect(event.defaultPrevented).toBe(false)
  await user.keyboard('{Enter}')
  expect(screen.queryByRole('dialog', { name: '停止传输并退出？' })).not.toBeInTheDocument()
  expect(backend.startSSHSession).not.toHaveBeenCalled()
  await user.keyboard('{ArrowDown}')
  expect(within(screen.getByRole('dialog', { name: '快速连接' })).getAllByRole('row')[1]).toHaveAttribute('aria-selected', 'true')
})
