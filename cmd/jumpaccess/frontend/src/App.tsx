...
function AppContent({ backend = wailsBackend }: AppProps) {
  ...
  const transferControllers = useRef(new Map<string, ZmodemController>())
  const [transferStates, setTransferStates] = useState<Record<string, ZmodemState>>({})
  const earlyOutput = useRef(new Map<string, string>())

  function appendSSHOutput(id: string, data: string) {
    const tab = workspaceRef.current.tabs.find((item): item is SSHTab => item.kind === 'ssh' && item.sessionID === id)
    if (!tab) {
      earlyOutput.current.set(id, ((earlyOutput.current.get(id) ?? '') + data).slice(-terminalBufferLimit))
      if (earlyOutput.current.size > 64) earlyOutput.current.delete(earlyOutput.current.keys().next().value!)
      return
    }
  ...
  const [refreshingProfiles, setRefreshingProfiles] = useState<Set<string>>(() => new Set())
  ...
  useEffect(() => {
    ...
    const offState = backend.onSessionState((event) => {
      ...
      if (event.status === 'closed' || event.status === 'failed') {
        transferControllers.current.get(event.id)?.dispose()
        transferControllers.current.delete(event.id)
        setTransferStates(current => { const next = { ...current }; delete next[event.id]; return next })
      }
    ...
    const offOutput = backend.onSessionOutput((event) => {
      const controller = transferFor(event.id)
      const acknowledge = () => { if (event.sequence) void backend.acknowledgeSSHOutput?.(event.id, event.sequence).catch(() => {}) }
      if (controller) void controller.consume(event.encoding === 'base64' ? decodeBytes(event.data) : new TextEncoder().encode(event.data)).finally(acknowledge)
      else { appendSSHOutput(event.id, event.encoding === 'base64' ? new TextDecoder().decode(decodeBytes(event.data)) : event.data); acknowledge() }
    })
    ...
    return () => {
      ...
      for (const controller of transferControllers.current.values()) controller.dispose()
      transferControllers.current.clear()
      earlyOutput.current.clear()
    }
  }, [backend])
  ...
  }, [backend, currentProfileLoggedIn, profile, currentProfile?.url, organizationRefreshKey])
  ...
  async function beginSSHConnection(tabID: string, descriptor: SSHDescriptor, reconnecting: boolean) {
    ...
    try {
      ...
      appendSSHOutput(session.id, '')
      if (session.status === 'active' && pendingState?.status !== 'active') void transferFor(session.id)?.probe()
      pendingSessionStates.current.delete(session.id)
      if (pendingState) {
  ...
  async function refreshProfileAuth(name: string) {
    if (refreshingProfiles.has(name)) return
    setRefreshingProfiles((current) => new Set(current).add(name))
    try {
      await run(async () => {
          ...
          <div className="content" hidden={activeTab?.kind !== 'assets'}>
            <section className="asset-pane">
              ...
              {!profile ? <EmptyState title="尚未创建 Profile" action="添加 Profile" onAction={() => { openSingleton('profiles'); setProfileDialog(true) }} /> : <>
                <div className="asset-toolbar"><label className="search-box"><Search /><input autoCorrect="off" autoCapitalize="none" spellCheck={false} ref={searchRef} role="searchbox" aria-label="搜索资产或 Alias" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索名称、地址、Asset ID 或 Alias" /><kbd>/</kbd></label><AliasFilterMenu onChange={setAliasFilter} value={aliasFilter} /></div>
...
function QuickConnectDialog({ assets, details, detailError, onRetry, onCancel, onConnectAlias, onConnectAsset, query, setQuery }: {
  ...
  return <Modal title="快速连接" className="quick-connect-modal" onClose={onCancel} closeOnEscape>
      <label className="quick-search"><Search /><input autoCorrect="off" autoCapitalize="none" spellCheck={false} autoFocus role="combobox" aria-label="搜索资产或别名" aria-expanded="true" aria-haspopup="grid" aria-controls="quick-results" aria-activedescendant={selected ? `quick-result-${selectedIndex}` : undefined} aria-describedby="quick-keyboard-help" value={query} onChange={(event) => { setSelection({ query: event.target.value, key: '' }); setQuery(event.target.value) }} placeholder="名称、地址、Asset ID 或 Alias" /></label>
