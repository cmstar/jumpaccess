...
export function ZmodemToolbar({ state, active, onCommand }: { state?: ZmodemState; active: boolean; onCommand: (command: string) => void }) {
  ...
  const canUpload = active && !!state?.upload && !state.busy
  const canDownload = active && !!state?.download && !state.busy
  return <>
    <button aria-label="上传文件（ZMODEM）" className="icon-button" disabled={!canUpload} onClick={() => onCommand('rz\r')} title={canUpload ? '上传文件到当前目录（rz / ZMODEM）' : '上传文件到当前目录（rz / ZMODEM 当前不可用）'} type="button"><Upload /></button>
    <button aria-label="下载文件（ZMODEM）" className="icon-button" disabled={!canDownload} onClick={() => { setError(''); setDownload(true) }} title={canDownload ? '下载文件（sz / ZMODEM）' : '下载文件（sz / ZMODEM 当前不可用）'} type="button"><Download /></button>
    {download ? <div className="modal-backdrop" onKeyDown={event => { if (event.key === 'Escape') setDownload(false) }}>
      <form aria-label="下载远程文件" aria-modal="true" className="modal" role="dialog" onSubmit={event => {
        ...
        <div className="dialog-actions"><button className="button secondary" onClick={() => setDownload(false)} type="button">取消</button><button className="button primary" disabled={!canDownload} type="submit"><Download />下载</button></div>
      </form>
    </div> : null}
  </>
}
