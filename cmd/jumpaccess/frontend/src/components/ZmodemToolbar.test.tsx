...
test.each([true, false])('未检测到命令时隐藏按钮（检测完成：%s）', checked => {
  render(<ZmodemToolbar active state={{ checked, upload: false, download: false, busy: false }} onCommand={vi.fn()} />)
  expect(screen.queryByLabelText('上传文件（ZMODEM）')).not.toBeInTheDocument()
  expect(screen.queryByLabelText('下载文件（ZMODEM）')).not.toBeInTheDocument()
})

test('按方向显示按钮，传输期间禁用，下载路径使用安全的字面量参数', () => {
