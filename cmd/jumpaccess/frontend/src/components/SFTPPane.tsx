...
export function SFTPPane({ backend, tab, active = true, onReconnect, onDisconnect }: SFTPPaneProps) {
  ...
  function selectEntry(event: MouseEvent<HTMLTableCellElement>, entryPath: string) {
    const start = namePointerStart.current
    namePointerStart.current = null
    if (event.detail > 1) return
    // 拖选后也会触发 click；按本次按下与松开的位置区分，避免旧文本选区挡住下一次单击。
    if (event.detail > 0 && start && (Math.abs(event.clientX - start.x) > 3 || Math.abs(event.clientY - start.y) > 3)) return
    setSelected((current) => current.includes(entryPath) ? current.filter((path) => path !== entryPath) : [...current, entryPath])
  }
