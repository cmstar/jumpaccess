...
export function TerminalBackgroundSettings({ backend, preferences, onChange }: { backend: Backend; preferences: Preferences; onChange: (value: TerminalBackground) => void }) {
  ...
  function slider(label: string, key: 'transparencyPercent' | 'positionXPercent' | 'positionYPercent', max: number, help: string) {
    const id = `terminal-background-${key}`
    return <div className="terminal-style-row"><div><label htmlFor={id}>{label}</label><small className="setting-help" id={`${id}-help`}>{help}</small></div><div className="background-slider"><input id={id} aria-describedby={`${id}-help`} type="range" min="0" max={max} step="1" value={draft[key]} onChange={event => update({ [key]: Number(event.target.value) }, false)} onPointerUp={commit} onPointerCancel={commit} onKeyUp={commit} onBlur={commit} /><output htmlFor={id}>{draft[key]}%</output></div></div>
  }
  return <section className="settings-card" id="settings-terminal-background">
    ...
    <div className="terminal-style-row">
      <div>
        <label htmlFor="terminal-background-path">背景图文件</label>
        <small className="setting-help" id="terminal-background-path-help">支持 PNG、JPG、WEBP、GIF，最大 20 MiB、4000 万像素。图片不可用时使用终端纯色背景。</small>
        {dialogError || error ? <p className="background-error" role="alert">{dialogError || error}</p> : loading ? <small role="status" className="setting-help">正在加载背景图…</small> : null}
      </div>
      <div className="background-file-control"><input id="terminal-background-path" aria-describedby="terminal-background-path-help" value={draft.filePath} placeholder="未选择图片" readOnly title={draft.filePath} /><button type="button" className="button secondary small" disabled={choosing} onClick={() => void choose()}>选择图片</button><button type="button" className="button ghost small" disabled={!draft.filePath || choosing} onClick={() => { setDialogError(''); update({ filePath: '', enabled: false }) }}>清除</button></div>
    </div>
    <div className="terminal-style-fields">
      {slider('图片透明度', 'transparencyPercent', 95, '0% 最显眼，95% 几乎不可见；默认 70%。文字透明度保持不变。')}
      ...
      {draft.fitMode === 'cover' ? <>{slider('水平位置', 'positionXPercent', 100, '50% 为居中；0% 靠左，100% 靠右。')}{slider('垂直位置', 'positionYPercent', 100, '50% 为居中；0% 靠顶部，100% 靠底部。')}</> : null}
