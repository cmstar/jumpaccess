...
- 连接后在同一 SSH 连接的独立命令通道执行 `command -v rz`、`command -v sz`（最多等待 5 秒）；查到哪个命令，就显示对应的上传或下载按钮。未找到时隐藏，探测失败时标记为未知，手工命令握手仍可启用对应能力。探测只检查命令可用性，不保证传输授权。
- 上传、下载按钮位于 SSH Tab 工具栏右侧，断开连接按钮始终位于最右侧，并用分隔线隔开。
