...
- 传输期间在终端内显示文件名、已传输大小和百分比，进度原地刷新；底部状态栏仍可取消。本地生成的终端提示统一使用英文，如 `Upload`、`Download`、`Saving` 和 `Complete`；文件名和服务器原始输出按原样显示。下载收到全部数据后显示 `Saving`，保存成功才显示 100% 和 `Complete`；普通终端输入暂时停用，切换 Tab 不影响传输。取消、错误或断连会清理未完成的本地下载。
