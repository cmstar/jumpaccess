...
- GUI 设置页采用左侧导航和右侧单列滚动内容：外观、终端、Tab 行为、关于 JumpAccess 分别占据独立面板并保留在同一页面；点击导航滚动到对应面板，滚动内容时导航同步选中当前面板，不为每项设置另开页面。应用主题归入外观；终端面板内部按“字体与配色”和“交互”分组；顶部工作区 Tab 的关闭入口和关闭确认归入 Tab 行为。
...
- SSH 终端默认把鼠标右键解释为粘贴剪贴板文本；用户可以在终端的“交互”设置中改为打开应用内上下文菜单。菜单包含复制和粘贴：没有终端选区时复制禁用，复制历史输出不要求 Session 仍然在线；只有活动 Session 可以粘贴。右键行为切换立即应用于已打开终端，不得为此重建终端或丢失当前选区。
...
- SSH 页面标题栏依次显示 Alias（没有时使用原始 Asset 名）、原始 Asset 名、Asset ID，以及连接状态灯和到 JumpServer SSH 网关的当前往返延迟。活动连接首次探测前显示灰灯与 `— ms`；延迟小于 `100 ms` 显示绿灯，`100–200 ms`（含边界）显示黄灯，大于 `200 ms` 显示橙灯；断开或失败只显示红灯，不显示延迟文字。标题栏右侧的断开按钮只终止可用的 live Session 并保留 Tab，不可用时禁用；关闭 Tab 仍由顶部 Tab 的关闭按钮负责。
