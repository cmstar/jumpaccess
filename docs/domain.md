...
- SSH Tab 支持会话内 ZMODEM 上传和下载；远端 `rz` 对应本机上传，`sz` 对应本机下载。命令可用性与堡垒机传输授权是独立条件，不从 SFTP 权限推断。分别检测到能力后，在工具栏右侧、断开按钮之前显示对应按钮；探测失败为未知，手工命令握手仍可启用对应能力。点击按钮前需处于命令提示符。下载按钮接受单个文件路径，不提供目录浏览；多文件可用手工 `sz file1 file2`。传输以 live Session 为单位，切换 Tab 不中断，重连后重新检测；取消、断连或退出会清理未完成的本地下载，同名文件自动另存，不覆盖已有文件。
...
- SSH 页面标题栏依次显示 Alias（没有时使用原始 Asset 名）、原始 Asset 名、Asset ID，以及连接状态灯和到 JumpServer SSH 网关的当前往返延迟。活动连接首次探测前显示灰灯与 `— ms`；延迟小于 `100 ms` 显示绿灯，`100–200 ms`（含边界）显示黄灯，大于 `200 ms` 显示橙灯；断开或失败只显示红灯，不显示延迟文字。标题栏右侧依次提供复制选中文本、粘贴剪贴板文本、复制当前工作目录、连接 SFTP（资产授权 SFTP 时显示）、ZMODEM 上传和下载（检测到对应命令时显示）、分隔线和最右侧的断开连接按钮；前两个按钮与终端上下文菜单共用动作，悬停分别提示 `复制选中文本 (Ctrl + Insert)` 和 `粘贴剪贴板文本 (Shift + Insert)`。没有选区时复制禁用，只有活动 Session 可以粘贴；断开按钮只终止可用的 live Session 并保留 Tab，不可用时禁用；关闭 Tab 仍由顶部 Tab 的关闭按钮负责。
