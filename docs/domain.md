...
- GUI 设置页采用左侧导航和右侧单列滚动内容：外观、终端样式、终端背景图、终端行为、Tab 行为、关于 JumpAccess 分别占据同级独立面板并保留在同一页面；点击导航滚动到对应面板，滚动内容时导航同步选中当前面板，不为每项设置另开页面。应用主题归入外观；终端样式包含公共预览、配色方案、字体、字号、行高及光标；终端行为包含鼠标右键与多行粘贴警告；顶部工作区 Tab 的关闭入口和关闭确认归入 Tab 行为。
