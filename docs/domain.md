...
- GUI 设置页采用左侧导航和右侧单列滚动内容：外观、终端样式、终端行为、Tab 行为、关于 JumpAccess 分别占据同级独立面板并保留在同一页面；点击导航滚动到对应面板，滚动内容时导航同步选中当前面板，不为每项设置另开页面。应用主题归入外观；终端样式包含公共预览、配色方案、字体与字号；终端行为包含鼠标右键与多行粘贴警告；顶部工作区 Tab 的关闭入口和关闭确认归入 Tab 行为。
...
- 公共预览使用隔离的只读 xterm 实例和固定演示文本，不建立 SSH 连接。字体、字号、配色更新不得重建现有终端或重新播放历史；保留已有输出与选区，字体尺寸变化时同步终端尺寸。连续修改偏好立即预览、按顺序保存；保存失败时报告错误，只有最新请求失败才恢复最后成功保存的偏好。SSH 终端背景必须铺满标题栏与状态栏之间的整个内容区；文字四周保留 12px 内边距，留白背景与终端底色完全一致，不露出应用背景色。字符网格未占满的余量和加载状态也使用终端配色。行高、光标样式和闪烁尚未实现，不显示设置项或占位说明；后续在终端样式面板中扩展，光标样式与闪烁放在一起。
