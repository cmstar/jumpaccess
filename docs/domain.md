...
- SSH 终端的多行粘贴警告默认开启，标题栏按钮、鼠标右键、上下文菜单、`Shift + Insert` 和原生 paste 事件共用同一检测规则。剪贴板文本包含 `CR` 或 `LF` 时，包括单条命令末尾自带换行，必须先以应用内 Modal 显示行数、字符数和可滚动的纯文本预览；取消、断连或 Session 切换不得写入终端，只有用户选择“仍然粘贴”后才按普通用户输入发送。用户可以在终端的“交互”设置中关闭警告，切换立即应用于已打开终端且不得重建终端；单行文本始终直接粘贴。
