...
- 终端配色通过一个按“深色 / 浅色”分组的单选下拉框选择，每项显示名称、完整 ANSI 16 色色表和选中标记。首批内置 14 个方案，默认 Nord；选择后自动保存到 GUI 偏好，公共预览与 SSH 终端使用同一配色和字体参数。配色与应用主题独立，只影响终端内容、光标和选区，不改变窗口、菜单、Tab 和连接状态栏；应用主题变化也不得覆盖终端方案。方案控制默认前景、背景与 ANSI 0–15 色，远端程序指定的 256 色扩展槽与 truecolor 颜色不被重映射。
